...
添加事件监控(图形, "click", (e) => {
  ...
  左 = Math.min(左, document.body.offsetWidth - 设置等级元素方位.width - 最小间距);
  左 = Math.max(左, 最小间距);

  let 上 = Math.round(省元素方位.top + 省元素方位.height / 2 - 设置等级元素方位.height / 2);
  上 = Math.min(上, document.body.offsetHeight - 设置等级元素方位.height - 最小间距);
  上 = Math.max(上, 最小间距);
...
const 保存图像 = (_) => {
  ...
  // return ;
  地址变图像元素(数据地址, (图) => {
    上下文.fillStyle = "#FAE8C9";
    上下文.fillRect(0, 0, 宽 * 比, 宽 * 比);
    上下文.drawImage(图, 0, 0, 宽, 高, 0, ((宽 - 高) * 比) / 2, 宽 * 比, 高 * 比);
